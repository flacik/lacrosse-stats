...
function seedDummyData() {
  try {
    var tourSheet  = getSheet(CONFIG.SHEET_TOURNAMENTS);
    var matchSheet = getSheet(CONFIG.SHEET_MATCHES);
    var eventSheet = getSheet(CONFIG.SHEET_EVENTS);
    ...
    function randomResult(zone) {
      ...
      return r < 0.02 ? 'gol' : r < 0.20 ? 'celny' : 'niecelny'; // own-half
    }
    ...
    matches.forEach(function(m) {
      ...
      periods.forEach(function(period) {
        var shotsA = 3 + Math.floor(Math.random() * 5); // 3–7 strzałów na drużynę na kwartę
        ...
        [[teamA, shotsA],[teamB, shotsB]].forEach(function(pair) {
          ...
          for (var i = 0; i < count; i++) {
            ...
            //             period, team_event, shot_x, shot_y, zone_name, result, man_up, man_down, created_at
            eventRows.push([
    ...
    // Batch write — znacznie szybsze niż appendRow w pętli
    ...
    var summary = 'Seed OK — turnieje: ' + tours.length +
                  ', mecze: ' + matches.length +
                  ', eventy: ' + eventRows.length;
    Logger.log(summary);
    return ok({ tournaments: tours.length, matches: matches.length, events: eventRows.length });
  } catch (e) {
