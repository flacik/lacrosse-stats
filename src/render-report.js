...
function openMatchReport(matchId) {
  ...
  var svgA = _reportHalfFieldSvg(shotEvents, match.team_A);
  var svgB = _reportHalfFieldSvg(shotEvents, match.team_B);
...
function openAnalyticsReport() {
  ...
  var svgZone = f.team ? _buildZoneEfficiencySvg(filtered) : '';
  ...
  var html = _reportShell('Analityka — ' + escapeHtml(teamLabel), subtitle, [
    ...
    svgZone ? _sectionAnalyticsShotChart(svgZone, f.team) : '',
...
function _sectionAnalyticsShotChart(svgZone, teamName) {
  return '<div class="section">' +
    '<h2>Skuteczność stref — ' + escapeHtml(teamName) + '</h2>' +
    '<div class="chart-svg chart-svg-single">' + svgZone + '</div>' +
  '</div>';
}
...
function _reportHalfFieldSvg(shotEvents, teamName) {
  var ns  = 'http://www.w3.org/2000/svg';
  var svg = document.createElementNS(ns, 'svg');
  svg.setAttribute('viewBox', '0 0 540 660');
  svg.setAttribute('xmlns', ns);
  var mockMatch  = { id: '__report__', team_A: teamName, team_B: '__other__', team_A_side: 'left' };
  var mockEvents = shotEvents.map(function(e) { return Object.assign({}, e, { team_event: teamName }); });
  var mockViewer = { view_mode: 'half-A', display_mode: 'heatmap' };
...
function _reportCss() {
  return [
    ...
    '.chart-svg-single{max-width:240pt;margin:0 auto}',
    '.chart-svg-single svg{width:100%;height:auto}',
  ].join('\n');
}
